...
function handleClickOnChatContainer(chatbot) {
	document.body.addEventListener("click", (event) => {
		...
		if (target) {
			...
			if (link.startsWith("#")) {
				// Si le lien commence par "link:http" ou "link:mailto", alors c'est un lien vers une URL externe ou vers une messagerie, et on l'ouvre dans un nouvel onglet
				if (link.startsWith("#link:http") || link.startsWith("#link:mailto")) {
					event.preventDefault();
